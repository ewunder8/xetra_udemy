...
class XetraSourceConfig(NamedTuple):
    ...
    src_first_extract_date: determines the dates for extracting the source
...
class XetraTargetConfig(NamedTuple):
    """
    trg_col_isin: column name for insin in target
    trg_col_date: column name for date in target
    trg_col_op_price: column name for op price in target
...
class XetraETL():
    ...
    def __init__(self, s3_bucket_src: S3BucketConnector,
        ...
        :param meta_key: used as self.meta_key -> key to meta file
        ...
        self.extract_date,  self.extact_date_list = MetaProcess.return_date_list(
            self.src_args.src_first_extract_date, self.meta_key, self.s3_bucket_trg)
        self.meta_update_list = [date for date in self.extact_date_list
                                 if date >= self.extract_date]

    def extract(self):
        """
        Read the source data and concatenates them to on dataframe
        ...
        self._logger.info('Extracting Xetra source files finsihed.')
        return data_frame
    ...
    def etl_report1(self):
        """
        Extact, transform and load to create report 1
        """
        # Extraction
